...
##### 3.1.3.2 比较次数下界：$\sum_{k=1}^{\log n} \Theta(n \cdot h_k) = \Theta(n^2)$
...
##### 3.2.2.1 关键观察：$h_{k+2} = 2h_{k+1} + 1$保证$\gcd(h_{k+1}, h_{k+2}) = 1$
...
##### 3.2.2.2 Frobenius硬币问题：$(h_{k+1}-1)(h_{k+2}-1)$作为表示阈值
...
##### 4.3.2.1 Plaxton-Poonen-Suel结果：特定条件下的下界
