...
/**
 * Toolbox Pages (Next.js App Router 迁移版)
 *
 * 迁移自旧 Vite 项目 src/pages/Tools/index.tsx：
 * - react-router (useNavigate/useParams) → next/navigation (useRouter)
 * - /tools        → <ToolboxPage />   工具网格 + 搜索 + 分类过滤
 * - /tools/[id]   → <ToolDetailPage toolId={id} />  单个工具详情
 * - 工具 id 不存在时优雅降级：提示 + 返回工具箱
 *
 * 数据层不变：工具全部为本地纯客户端实现，无 public 数据依赖。
 *
 * @author SAKURAIN
 */
  ...
  if (!tool) {
    return (
      <div className="text-center py-20">
        ...
        <p style={{ color: 'var(--text-muted)' }}>它可能已被移除或更名</p>
        <button
          onClick={onBack}
          className="mt-6 px-4 py-2 inline-flex items-center gap-2"
          style={{
...
export function ToolDetailPage({ toolId }: { toolId: string }) {
  ...
  // Global keyboard shortcuts（仅详情页启用）
